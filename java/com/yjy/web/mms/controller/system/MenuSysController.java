...
public class MenuSysController {
	...
	public String newpage(Model model,HttpServletRequest req) {
		...
		if (!StringUtils.isEmpty(req.getParameter("id"))) {
			...
			SystemMenu menuObj = iDao.getOne(getId);
	...
	public String testMess(HttpServletRequest req, @Valid SystemMenu menu, BindingResult br) {
		...
		System.out.println("getAdd:"+session.getAttribute("getAdd"));
		System.out.println("getName:"+session.getAttribute("getName"));
		System.out.println("isroot:"+req.getParameter("isroot"));
		...
		// 校验通过，下面写自己的逻辑业务
			...
			if (!StringUtils.isEmpty(session.getAttribute("getId"))) {
				...
				if(req.getParameter("isroot").toString().equals("0")) {
					menu.setParentId(0L);
					menu.setMenuUrl("#");
				}
				//执行新增 的代码；

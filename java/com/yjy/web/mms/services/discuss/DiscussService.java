...
public class DiscussService {
	...
	public List<Map<String, Object>> packaging(List<Discuss> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			result.put("typeName", typeDao.findById(list.get(i).getTypeId()).orElse(null).getTypeName());
			...
			result.put("typecolor", typeDao.findById(list.get(i).getTypeId()).orElse(null).getTypeColor());
			listMap.add(result);
		}
		return listMap;
	}

package com.yjy.web.mms.services.medicalmattersservices;
...
@Service
@Transactional
public class MedicalMattersServices {
	@Autowired
...
//			pro.setProFileids(attachments);
	...
	/**
		 ...
		 * 
		 */
		public void  getApplyDictionaries(Model model,Long id,int page,int size){
			// 查询部门表
			Iterable<Dept> deptlist = ddao.findAll();
			// 查职位表
			Iterable<Position> poslist = pdao.findAll();
		 ...
		 * 
		 */
		public void setMedicalMatters(MedicalMattersList medicalMattersList,String val,User lu,String name) {
			medicalMattersList.setTypeNmae(val);
			medicalMattersList.setApplyTime(new Date());
			medicalMattersList.setUserId(lu);
			medicalMattersList.setStatusId(25L);
			medicalMattersList.setShenuser(name);
			medicalMattersList.setUserId(lu);
		 ...
		 * 1、设置新提交审核表内容
		 */
		public void setMedicalMattersReviewed(User reuser,MedicalMattersList mmpro){
			MedicalMattersReviewed mmrevie=new MedicalMattersReviewed();
			mmrevie.setUserId(reuser);
			mmrevie.setStatusId(23L);
			mmrevie.setMedicalmattersId(mmpro);
			mmredao.save(mmrevie);
		}
}

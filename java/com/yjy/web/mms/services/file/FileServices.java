package com.yjy.web.mms.services.file;
...
import com.yjy.web.mms.model.entity.file.FileList;
import com.yjy.web.mms.model.entity.file.FilePath;
...
public class FileServices {
	...
	public void findAllParent(FilePath filePath,List<FilePath> parentpaths){
		if (filePath.getParentId() != 1L) {
			FilePath filepath1 = fpdao.findById(filePath.getParentId()).orElse(null);
...
//			System.out.println("此文件下下再无文件夹");
	...
	public void filereturnback(List<Long> checkfileids,Long userid) {
		...
		for (Long checkfileid : checkfileids) {
			FileList fileList = fldao.findById(checkfileid).orElse(null);
		...
		if(fromwhere){
			...
			if(!mcpathids.isEmpty()){
				...
				for (Long mcpathid : mcpathids) {
					FilePath filepath = fpdao.findById(mcpathid).orElse(null);
	...
	public void rename(String name,Long renamefp,Long nowpathid,boolean isfile){
		if(isfile){
			//文件名修改
			FileList fl = fldao.findById(renamefp).orElse(null);
			String newname = onlyname(name, fl.getFpath(), fl.getFileShuffix(), 1, isfile);
			fl.setFileName(newname);
			fldao.save(fl);
		}else{
			//文件夹名修改
			FilePath fp = fpdao.findById(renamefp).orElse(null);
			FilePath filepath = fpdao.findById(nowpathid).orElse(null);

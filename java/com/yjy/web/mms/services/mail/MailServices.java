package com.yjy.web.mms.services.mail;
...
public class MailServices {
	...
	public List<Map<String,Object>> mail(Page<Pagemail> mail){
		...
		for (int i = 0; i < maillist.size(); i++) {
			...
			SystemStatusList status=sdao.findById(maillist.get(i).getMailStatusid()).orElse(null);
	...
	public List<Map<String, Object>> up(Page<Mailnumber> num){
		...
		for (int i = 0; i < account.size(); i++) {
			Map<String, Object> result=new HashMap<>();
			SystemStatusList status=sdao.findById(account.get(i).getStatus()).orElse(null);
